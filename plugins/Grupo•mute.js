...
const handler = async (m, { conn, command, text, isAdmin }) => {
  ...
  if (user === ownerNumber)
    return conn.reply(m.chat, "🚩 *No puedes mutar al creador del bot*", m);

  const groupMetadata = await conn.groupMetadata(m.chat);
  const groupOwner =
    groupMetadata.owner || m.chat.split`-`[0] + "@s.whatsapp.net";
  if (user === groupOwner)
    return conn.reply(m.chat, "🚩 *No puedes mutar al creador del grupo*", m);
  ...
  const fkontak = {
    ...
    message: {
      locationMessage: {
        name: command === "mute" ? "𝗨𝘀𝘂𝗮𝗿𝗶𝗼 𝗺𝘂𝘁𝗮𝗱𝗼" : "𝗨𝘀𝘀𝘂𝗮𝗿𝗶𝗼 𝗱𝗲𝗺𝘂𝘁𝗮𝗱𝗼",
  ...
  if (command === "mute") {
    ...
    conn.reply(m.chat, "*Tus mensajes serán eliminados*", fkontak, null, {
      mentions: [user],
    });
  }

  if (command === "unmute") {
    if (userData.mute === false || !userData.mute)
      return conn.reply(m.chat, "🚩 *Este usuario no ha sido mutado*", m);
    if (user === m.sender)
      return conn.reply(
        m.chat,
    ...
    conn.reply(m.chat, "*Tus mensajes no serán eliminados*", fkontak, null, {
      mentions: [user],
    });
  }
};
